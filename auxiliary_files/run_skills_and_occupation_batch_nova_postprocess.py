...
# #     """
# #     Process downloaded .jsonl.out files to extract occupation codes and save as CSV.
...
# #                                     # Extract JSON within triple backticks
...
# #                                         # Ensure occupation is a list of strings
# #                                         if isinstance(occupation, list):
# #                                             occupation = [str(code) for code in occupation]
# #                                         else:
# #                                             occupation = []
# #                                         records.append({
# #                                             'job_guid': record_id,
...
# #                                         })
# #                                     else:
# #                                         print(f"[WARNING] No JSON content found in line: {line}")
# #                                 else:
# #                                     print(f"[WARNING] Unexpected content format in line: {line}")
# #                             except json.JSONDecodeError as jde:
# #                                 print(f"[ERROR] JSON decode error in file {file_path}: {jde}")
# #                             except Exception as e:
# #                                 print(f"[ERROR] Unexpected error processing line in {file_path}: {e}")
...
# #     session_id = '20250121-140509-0244d'
...
# #         # You can add a new entry in your config for local output folder if preferred
...
# def post_process_results(raw_results: str, task: str) -> list:
#     """
#     Processes the raw_results string by removing code fences and extracting the specified task data.

#     Args:
#         raw_results (str): The raw string containing JSON data, possibly wrapped in code fences.
#         task (str): The task name to extract from the JSON data (e.g., 'occupation', 'skills').

#     Returns:
#         list: A list of items extracted from the JSON data based on the task.
#     """
#     try:
...
#         # 3. Extract the specified task field if it exists
#         if isinstance(parsed_value, dict) and task in parsed_value:
#             return parsed_value[task]
#         else:
#             print(f"[WARNING] No '{task}' field found in the parsed JSON.")
#             return []
#     except json.JSONDecodeError as e:
#         print(f"[ERROR] Failed to parse raw_results: {e}")
#         return []
...
#                 records = []
#                 try:
#                     # Extract task name from file name (first part before '_')
...
#                                         # Parse the extracted JSON string
#                                         json_data = json.loads(json_str)
#                                         # Extract the relevant task data
#                                         task_data = json_data.get(task_name, [])
#                                         # Ensure task_data is a list of strings
#                                         if isinstance(task_data, list):
#                                             task_data = [str(item) for item in task_data]
#                                         else:
#                                             task_data = []
#                                         records.append({
#                                             'job_guid': record_id,
...
def main():
    # Use the fixed session_id as per the original script
    session_id = '20250122-130455-3d03d'
